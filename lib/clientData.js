...
const ClientData = {
    ...
    clientItemsForStructure(structure) {
        ...
        // // Handling bundles can add duplicate client items if the same components are reused. Filter by size.
        // if(clientItems.bundleComponents){
        //     clientItems.bundleComponents = _.uniqBy(clientItems.bundleComponents, (item) => item.size);
        //     clientItems.wires = _.uniqBy(clientItems.wires, (item) => item.size);
        //     clientItems.bundles = _.uniqBy(clientItems.bundles, (item) => item.size);
        // }
    ...
    clientItemsForType(type, component) {
        ...
        } else if ((lookupType === "wires") && component.clientItem && (component.clientItem.messenger !== undefined)) {
            let clientItem = component.clientItem;
            let clientBundle = [this.find('bundles', { size: clientItem.size })];
            mergeClientItems(clientItems, { bundles: clientBundle });
            mergeClientItems(clientItems, { wires: clientBundle});
            //TODO need to extract and merge bundle components with bundles.
            //TODO bundles should also be wires?
            for(let clientBundleComponent of clientItem.bundleComponents) {
                mergeClientItems(clientItems, { bundleComponents: [this.find('bundleComponents', {
                    size: clientBundleComponent.size})]
                });
            }
            // console.log({clientItems: clientItems})
            // clientItems["bundles"] = [clientItem];
            // clientItems["wires"] = [clientItem];
            // clientItems["bundleComponents"] = clientItem.bundleComponents;
    }
        else {
