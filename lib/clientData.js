...
const ClientData = {
    ...
    clientItemsForType(type, component) {
        ...
        if (lookupType === 'assemblies') {
            ...
            mergeClientItems(clientItems, {bundles: [this.find('bundles', {size: clientItem.size})]});
            mergeClientItems(clientItems, {wires: [this.find('wires', {size: clientItem.messenger.size})]});
            for (let clientBundleComponent of clientItem.bundleComponents) {
                mergeClientItems(clientItems, {
                    bundleComponents: [this.find('bundleComponents', {size: clientBundleComponent.size})]});
            }
        } else {
